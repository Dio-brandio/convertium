...
import type { Action, CompressAction } from "@/types/action";
...
const extensions = {
  ...
  video: [
    ...
    {
      type: "quarter",
      badge: "Can reduce quality highly",
      className: "text-red-700",
    },
    {
      type: "fifth",
...
export default function CompressFiles() {
  ...
  const [actions, setActions] = useState<CompressAction[]>([]);
  ...
  const download = (action: CompressAction) => {
  ...
  const compress = async (): Promise<any> => {
    ...
    for (let action of tmp_actions) {
      try {
        const { url, output } = await compressFile(ffmpegRef.current, action);
  ...
  const updateAction = (file_name: String, compresionLevel: string) => {
    setActions(
      actions.map((action): CompressAction => {
        if (action.file_name === file_name) {
          console.log("FOUND", { file_name, compresionLevel });
          return {
            ...action,
            compresionLevel,
          };
  ...
  const checkIsReady = (): void => {
    ...
    actions.forEach((action: CompressAction) => {
      if (!action.compresionLevel) tmp_is_ready = false;
    });
    setIsReady(tmp_is_ready);
  };
  const deleteAction = (action: CompressAction): void => {
  ...
  if (actions.length) {
    return (
      <div className="space-y-6">
        {actions.map((action: CompressAction, i: any) => (
          <div
            key={i}
          ...
          >
            ...
            {action.is_error ? (
              <Badge variant="destructive" className="flex gap-2">
                <span>Error Compressing File</span>
                <BiError />
              </Badge>
                ...
                <Select
                  onValueChange={(value) => {
                    if (extensions.audio.includes(value)) {
                      ...
                      extensions.video.find(({ type }) => type === value)
                    ) {
                    ...
                    {action.file_type.includes("video") && (
                      <div className="">
                        {extensions.video.map((elt, i) => (
                            <SelectItem key={i} value={elt.type} className={``}>
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger className={`mx-auto ${elt.className}`}>
                                    {elt.type}
                                  </TooltipTrigger>
                                  <TooltipContent className={`text-center ml-8`}>
                                    <p>${elt.badge}</p>
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
